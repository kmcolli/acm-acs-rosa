...
Currently, you cannot create and managed the cluster lifecycle of ROSA clusters, however you can import ROSA clusters into ACM.  After a ROSA cluster is imported into ACM you can then use ACM to manage applications and policies for that ROSA cluster like any other cluster.
...
You wil have a different value, but it should have returned something.

If you don't have this environment variable set, go back through the Create Admin User section of the ROSA demo.
...
*Output*
```bashcustomresourcedefinition.apiextensions.k8s.io/klusterlets.operator.open-cluster-management.io created
